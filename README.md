...
The current build pulls visual direction from `references/`. Game sprites live in `assets/generated/ballseat-sprite-sheet.png`, generated with the `imagegen` skill from the reference art, then used by the canvas renderer for Mommy Ball, Open the Closet, Goatbox, Slurp Slurp, Frank, and ticket pickups. The town backdrop is also generated art at `assets/generated/ballseat-town-background.png`.
